...
//  cppEx13_8
//
//  Created by DongJae Lee on 2021/06/01.
//
...
class InvalidputException : public MyException{
public:
    InvalidputException(int lineNo, string func, string msg):MyException(lineNo, func, msg){}
};
...
int main(){
    ...
    try{
        ...
        cin >> x >> y;
        if(x<0 || y<0)
            throw InvalidputException(32,"main()", "음수 입력 예외 발생");
        if(y == 0)
